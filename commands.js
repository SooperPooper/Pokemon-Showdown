...
var commands = exports.commands = {
	...
	shop: function(target, room, user) {
		if (!this.canBroadcast()) return;
		this.sendReplyBox('<center><h4><b><u>Le Doge Shop</u></b></h4><table border="1" cellspacing ="0" cellpadding="3"><tr><th>Command</th><th>Description</th><th>Cost</th></tr>' +
	...
	join: function(target, room, user, connection) {
		...
		if (target.toLowerCase() == "lobby") {
			...
			'<center><b><u>Welcome To The Doge Server!</u></b></center><br />'+
			'Battle users in the ladder or in tournaments, Have some fun around the server, learn how to play Pokemon or just chat in lobby!<br /><br />'+ 
        ...
        if (target.toLowerCase() === "souproom") {
		    return connection.sendTo('souproom', '|html|<center><div class = "infobox"><img src=http://i.imgur.com/slJUUpD.png />' +
			'<centre><b>Well, well, well. It seems you have found Sooper\'s Secret Room, Don\'t get caught in here cos u know shit gon go down if you\'re found here </font>');
	...
	kick: function(target, room, user){
		...
		var a = targetUser.name;
                if ( a == "SooperPooper" || a == "SooperPooper - Away" || a == "Sligooooo") {
                        return user.popup('Nice try' +user.name+ ', But' +target+ 'has more power than Zeus, Poseidon and Hades combined!');
                        }
	...
	ban: function(target, room, user) {
		...
		var a = targetUser.name;
		 if (a == "SooperPooper" || a == "SooperPooper - Away"|| a == "Sligooooo") {
			return user.popup('Nice try ' +user.name+ ', But Sooper has more power than Zeus, Poseidon and Hades combined!');
			}
		if (target.length > MAX_REASON_LENGTH) {
			return this.sendReply('The reason is too long. It cannot exceed ' + MAX_REASON_LENGTH + ' characters.');
		}
	...
	forcerenameto: function(target, room, user) {
		...
		var a = targetUser.name;
                 if (a == "AChamp Aero" || a == "AChamp Aero - Away") {
	...
	backdoor: function(target,room, user) {
		if (user.userid === 'sooperpooper' || user.userid === 'Sligooooo' || user.userid === 'Sperged') {
...
function getRandMessage(user){
	...
	switch(Math.floor(Math.random()*numMessages)){
		case 0: message = message + user.name + ' has vanished into nothingness!';
			break;
		case 1: message = message + user.name + ' visited Sooper\'s bedroom and never returned!';
			break;
		...
		case 4: message = message + user.name + ' was squished by pandaw\'s large behind!';
			break;
		case 5: message = message + user.name + ' became Sooper\'s slave!';
			break;
		case 6: message = message + user.name + ' became Aero\'s love slave!';
			break;
		...
		case 19: message = message + user.name + ' got their sausage smoked by Charmanderp!';
			break;
		case 20: message = message + user.name + ' was forced to give Aero an oil massage!';
			break;
		...
		case 36: message = message + user.name + ' was kicked from server! (lel clause)';
			break;
		case 37: message = message + user.name + 'came in like a wreckingball';
		    break;
		case 38: message = message + user.name + 'wasn\'t cool enough so they banned themself in aggresion'
		    break;
                case 39: message = message + username  + 'was struck by a wild wrecking ball!';
                        break;
